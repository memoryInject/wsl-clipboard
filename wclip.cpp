...
int wmain(int argc, wchar_t* argv[]) {
  ...
  // If we pass argument like './wclip.exe -i "Hello World!"' or 'wclip.exe
  // --input "Hello World!"' set the clipboard content to passed 3rd argument:
  // "Hello World!"
  ...
  if (argc == 2 && (help_args.find(argv[1]) != help_args.end())) {
    std::cout << "usage: wclip.exe [<flags>]\n\n"
              << "WSL-Clipboard Utility\n\n"
              << "Blazingly fast Windows clipboard utility for WSL "
                 "applications and Neovim running under WSL\n\n"
              << "\n\n"
              << "Flags:\n"
              << "  -h, --help                  Show help\n"
